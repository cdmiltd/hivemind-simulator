...
# 镜像版本号:构建时通过 --build-arg APP_VERSION 注入(来源 pom.xml 的 project.version),
# docker-build.ps1 自动提取并注入;直接 docker compose build 时默认 unknown
...
# apt 源替换为阿里云镜像:境内构建环境访问 archive.ubuntu.com 超时,
# 基础镜像为 Ubuntu(deb822 格式,配置位于 /etc/apt/sources.list.d/ubuntu.sources)
# 注:曾用清华源,但 2026-08-21 出现 502 Bad Gateway 故障(IP 101.6.15.130),
#     libmp3lame0/libx11-6 等包下载失败,故切换至阿里云源(mirrors.aliyun.com)
