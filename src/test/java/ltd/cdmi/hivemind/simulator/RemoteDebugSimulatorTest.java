...
class RemoteDebugSimulatorTest {
    ...
    void dock1_putterOpen_returnsResult0_schedulesProgressEvents_syncsState() throws Exception {
        ...
        // Job 指令需先进入远程调试模式（debug_mode_open，对齐真机 check_work_mode 检查）
    ...
    // ==================== TC-RD-013: debug_mode_open/close 联动 mode_code ====================

    /**
     * TC-RD-013：debug_mode_open → debugMode=true 且 dockModeCode=2（远程调试）
     * <p>实证回归：原实现只设内部 debugMode 标志，机场 OSD mode_code 保持 0（空闲中），
     * 平台无法感知调试模式状态变化。</p>
     */
    @DisplayName("TC-RD-013：debug_mode_open 联动机场 OSD mode_code=2（远程调试）")
    @Test
    void debugModeOpen_syncsDebugModeAndDockModeCode() {
        DeviceState state = new DeviceState();
        assertEquals(0, state.getDockModeCode(), "初始 mode_code 应为 0（空闲中）");
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        RemoteDebugSimulator simulator = createSimulator(DockModel.DOCK3, state, mqtt);

        Map<String, Object> result = simulator.handle("debug_mode_open", null, "bid-debug-open");

        assertEquals(0, result.get("result"));
        assertTrue(state.isDebugMode(), "debug_mode_open 后 debugMode 应为 true");
        assertEquals(2, state.getDockModeCode(), "debug_mode_open 后机场 OSD mode_code 应为 2（远程调试）");
        Mockito.verifyNoInteractions(mqtt); // Cmd 同步指令无进度事件（TC-RD-009）
    }

    /**
     * TC-RD-013：debug_mode_close → debugMode=false 且 dockModeCode=0（空闲中）
     */
    @DisplayName("TC-RD-013：debug_mode_close 联动机场 OSD mode_code=0（空闲中）")
    @Test
    void debugModeClose_syncsDebugModeAndDockModeCode() {
        DeviceState state = new DeviceState();
        // 预设已进入调试模式
        state.setDebugMode(true);
        state.setDockModeCode(2);
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        RemoteDebugSimulator simulator = createSimulator(DockModel.DOCK3, state, mqtt);

        Map<String, Object> result = simulator.handle("debug_mode_close", null, "bid-debug-close");

        assertEquals(0, result.get("result"));
        assertFalse(state.isDebugMode(), "debug_mode_close 后 debugMode 应为 false");
        assertEquals(0, state.getDockModeCode(), "debug_mode_close 后机场 OSD mode_code 应恢复 0（空闲中）");
