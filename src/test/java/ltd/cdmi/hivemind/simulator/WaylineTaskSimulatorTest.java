...
class WaylineTaskSimulatorTest {
    ...
    /** 反射调用 handleWaylineCommand（私有方法，bid=null 模拟平台未填） */
    @SuppressWarnings("unchecked")
    private Map<String, Object> invokeCommand(WaylineTaskSimulator simulator, String method, JsonNode data)
            throws Exception {
        return invokeCommand(simulator, method, data, null);
    }

    /** 反射调用 handleWaylineCommand（私有方法，三参数：method, data, bid） */
    @SuppressWarnings("unchecked")
    private Map<String, Object> invokeCommand(WaylineTaskSimulator simulator, String method, JsonNode data, String bid)
            throws Exception {
        Method m = WaylineTaskSimulator.class.getDeclaredMethod(
                "handleWaylineCommand", String.class, JsonNode.class, String.class);
        m.setAccessible(true);
        return (Map<String, Object>) m.invoke(simulator, method, data, bid);
    }
    ...
    @DisplayName("TC-WAYLINE-027：flighttask_progress 的 bid 与任务下发指令的 bid 一致（jobId 关联）")
    @Test
    void publishProgressBidMatchesCommandBid() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        ServiceCommandHandler commandHandler = Mockito.mock(ServiceCommandHandler.class);
        ...
        // 模拟平台下发 flighttask_execute，services 消息的 bid=jobId
        JsonNode data = objectMapper.readTree("{\"flight_id\":\"FLIGHT-BID-001\"}");
        Map<String, Object> reply = invokeCommand(simulator, "flighttask_execute", data, "job-abc-123");
        assertEquals(0, reply.get("result"));

        // 手动触发一次进度上报（不依赖异步调度时序）
        simulator.publishProgress("in_progress", 1, 30);

        // 捕获 events 消息，过滤 flighttask_progress（异步调度与手动上报均可能产生多条）
        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);
        Mockito.verify(mqtt, Mockito.atLeastOnce()).publish(Mockito.anyString(), captor.capture());
        JsonNode progressEvent = null;
        for (String payload : captor.getAllValues()) {
            JsonNode n = objectMapper.readTree(payload);
            if ("flighttask_progress".equals(n.path("method").asText())) {
                progressEvent = n;
            }
        }
        assertNotNull(progressEvent, "应至少上报一条 flighttask_progress");

        assertEquals("job-abc-123", progressEvent.path("bid").asText(),
                "进度事件的 bid 应与下发指令的 bid（jobId）一致，而非随机 UUID");
        assertFalse(progressEvent.path("tid").asText().isEmpty());
        assertNotEquals("job-abc-123", progressEvent.path("tid").asText(),
                "tid 是消息 id，不应与 bid（业务 id）相同");
    }

    @DisplayName("TC-WAYLINE-027：bid 缺失时（平台未填）fallback 随机 UUID，不为空")
    @Test
    void publishProgressBidFallbackWhenCommandBidMissing() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        ServiceCommandHandler commandHandler = Mockito.mock(ServiceCommandHandler.class);
        ...
        // 平台未填 bid（null）
        JsonNode data = objectMapper.readTree("{\"flight_id\":\"FLIGHT-BID-002\"}");
        invokeCommand(simulator, "flighttask_execute", data, null);
        ...
        assertFalse(progressEvent.path("bid").asText().isEmpty(), "bid 缺失时应 fallback 随机 UUID，不能为空串");
        assertFalse(progressEvent.path("tid").asText().isEmpty());
    }
    ...
    void flighttaskExecuteParsesMultiDockTask() throws Exception {
        ...
        // 通过反射调用 handleWaylineCommand（bid=null）
        Method m = WaylineTaskSimulator.class.getDeclaredMethod("handleWaylineCommand", String.class, JsonNode.class, String.class);
        m.setAccessible(true);
        @SuppressWarnings("unchecked")
        Map<String, Object> result = (Map<String, Object>) m.invoke(simulator, "flighttask_execute", data, null);
