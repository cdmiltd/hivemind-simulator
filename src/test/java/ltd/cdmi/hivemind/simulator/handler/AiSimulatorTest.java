...
class AiSimulatorTest {
    ...
    @DisplayName("补充测试：drc_ai_identify_filter_set 空列表=全部禁用，应接受并生效")
    @Test
    void handleAiIdentifyFilterSetEmptyListDisablesAll() {
        // 先设置为 [2,3]，再下发空列表，应清空（全部禁用）而非保持旧值
        ObjectNode setup = objectMapper.createObjectNode();
        setup.putArray("filters").add(2).add(3);
        aiSimulator.handleAiIdentifyFilterSet(setup);

        ObjectNode empty = objectMapper.createObjectNode();
        empty.putArray("filters");
        Map<String, Object> result = aiSimulator.handleAiIdentifyFilterSet(empty);

        assertEquals(0, result.get("result"));
        assertTrue(aiSimulator.getFilters().isEmpty(), "空列表应生效（全部禁用）");
        verify(mqtt, times(2)).publishJson(anyString(), any());
    }

    @DisplayName("补充测试：drc_ai_identify_filter_set 未携带 filters 字段时保持原值")
    @Test
    void handleAiIdentifyFilterSetMissingFieldKeepsFilters() {
        ObjectNode setup = objectMapper.createObjectNode();
        setup.putArray("filters").add(2).add(3).add(4);
        aiSimulator.handleAiIdentifyFilterSet(setup);

        Map<String, Object> result = aiSimulator.handleAiIdentifyFilterSet(objectMapper.createObjectNode());

        assertEquals(0, result.get("result"));
        assertEquals(List.of(2, 3, 4), aiSimulator.getFilters());
    }
