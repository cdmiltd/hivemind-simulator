 ...
 *   <li>TC-DRC-056：drc_mode_enter 缺失 mqtt_broker 仍进入 DRC 模式（空值防御）</li>
...
class AuthFlowHandlerTest {
    ...
    // ==================== TC-DRC-056：drc_mode_enter 缺失 mqtt_broker 空值防御 ====================

    @Test
    @DisplayName("TC-DRC-056: drc_mode_enter 缺失 mqtt_broker 仍进入 DRC 模式（result=0 + drcState=2 + state 上报）")
    void drcModeEnterWithoutMqttBrokerStillEntersDrcMode() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        DeviceState state = Mockito.mock(DeviceState.class);
        AuthFlowHandler handler = new AuthFlowHandler(
        ...
        // 调试器场景：data 为空对象（无 mqtt_broker / hsi_frequency / osd_frequency）
        JsonNode data = objectMapper.readTree("{}");

        Map<String, Object> reply = handler.handle("drc_mode_enter", data, "bid-drc-enter-2");

        // 空值防御：不抛异常，返回 result=0
        assertNotNull(reply);
        assertEquals(0, reply.get("result"), "缺失 mqtt_broker 时 drc_mode_enter 应返回 result=0");

        // 仍进入 DRC 模式：drcState=2 + state topic 上报
        verify(state).setDrcState(2);
        JsonNode stateMsg = capturePublished(mqtt, objectMapper, STATE_TOPIC);
        assertEquals(2, stateMsg.path("data").path("drc_state").asInt());
    }
