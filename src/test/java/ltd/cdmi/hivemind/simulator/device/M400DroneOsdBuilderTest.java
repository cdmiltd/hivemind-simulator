...
class M400DroneOsdBuilderTest {
    ...
    private OsdContext ctx(DroneModel droneType) {
        ...
        // gimbal_pitch/roll/yaw 默认 0.0，zoom_factor 默认 7.0（与 DeviceState 字段初始化一致）
        Mockito.lenient().when(state.getGimbalPitch()).thenReturn(0.0);
        Mockito.lenient().when(state.getGimbalRoll()).thenReturn(0.0);
        Mockito.lenient().when(state.getGimbalYaw()).thenReturn(0.0);
        Mockito.lenient().when(state.getZoomFactor()).thenReturn(7.0);
    ...
    void osdContainsSimplifiedGimbalInfo() {
        ...
        // TC-PAYLOAD-027：负载属性以负载索引枚举值为 key（M400→"82-0-0"），与 payload_index 数值一致
        Map<String, Object> gimbal = (Map<String, Object>) osd.get("82-0-0");
        assertNotNull(gimbal, "应以负载索引 82-0-0 为 key 上报负载属性 struct");
        assertFalse(osd.containsKey("type_subtype_gimbalindex"), "禁止以文档 Column 名字面量为 key");
        // gimbal_pitch/roll/yaw/zoom_factor 从 state 读取（不再硬编码）
        assertEquals(0.0, gimbal.get("gimbal_pitch"));
        assertEquals(0.0, gimbal.get("gimbal_roll"));
        assertEquals(0.0, gimbal.get("gimbal_yaw"));
        assertEquals("82-0-0", gimbal.get("payload_index"), "payload_index 应与 key 数值一致（H30 相机索引 82-0-0）");
        // zoom_factor 从 state 读取，DeviceState 默认 7.0（DRC 变焦指令可更新）
        assertEquals(7.0, gimbal.get("zoom_factor"));
