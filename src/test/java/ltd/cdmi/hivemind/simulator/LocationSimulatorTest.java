...
class LocationSimulatorTest {
    ...
    private SimulatorController newController(RuntimeConfig runtimeConfig, DeviceState state) {
        return new SimulatorController(
                ...
                Mockito.mock(LocalFileUploadService.class),
                Mockito.mock(MediaSampleInitializer.class),
                runtimeConfig,
                testProps(),
                ...
                Mockito.mock(ltd.cdmi.hivemind.simulator.mqtt.DrcConnectionManager.class)
        );
    }
    ...
    // ==================== TC-LOC-017：机场位置变更联动无人机在舱位置同步 ====================

    /**
     * 无人机在舱（droneInDock=true）时，PUT /api/location 应同步更新 DeviceState 中的
     * droneLatitude/droneLongitude/droneElevation 为新机场位置、droneHeight=0。
     */
    @DisplayName("TC-LOC-017：在舱时机场位置变更联动无人机位置同步")
    @Test
    void updateLocationSyncsDronePositionWhenInDock() {
        LiveConfigStore store = Mockito.mock(LiveConfigStore.class);
        Mockito.when(store.load()).thenReturn(null);
        RuntimeConfig rc = new RuntimeConfig(testMqttProps(), testProps(), store);
        // 无人机初始在舱，位置=旧机场位置（30.67, 104.07, 500.0）
        DeviceState state = new DeviceState();
        ...
        assertEquals(121.47, result.get("longitude"));
        assertEquals(10.0, result.get("height"));
        // RuntimeConfig 机场位置已更新
        assertEquals(31.23, rc.getLocationLatitude());
        assertEquals(121.47, rc.getLocationLongitude());
        assertEquals(10.0, rc.getLocationHeight());
        // 无人机在舱，位置已联动同步为新机场位置
        assertEquals(31.23, state.getDroneLatitude());
        assertEquals(121.47, state.getDroneLongitude());
        assertEquals(10.0, state.getDroneElevation());
        // 在舱时无人机相对起飞点高度归零
    ...
    // ==================== TC-LOC-018：飞行中机场位置变更不干预无人机位置 ====================

    /**
     * 无人机飞行中（droneInDock=false）时，PUT /api/location 仅更新机场位置，
     * 不应修改 DeviceState 中由飞行模拟器控制的无人机位置。
     */
    @DisplayName("TC-LOC-018：飞行中机场位置变更不干预无人机位置")
    @Test
    void updateLocationDoesNotTouchDronePositionWhenInFlight() {
        LiveConfigStore store = Mockito.mock(LiveConfigStore.class);
        Mockito.when(store.load()).thenReturn(null);
        RuntimeConfig rc = new RuntimeConfig(testMqttProps(), testProps(), store);
        // 无人机飞行中，位置偏离机场（航线 step 2 偏移态）
        DeviceState state = new DeviceState();
        state.setDroneInDock(false);
        state.setDroneActivated(true);
        state.setDroneModeCode(5); // 航线飞行
        state.setDroneLatitude(30.671);
        state.setDroneLongitude(104.071);
        ...
        assertEquals(121.47, rc.getLocationLongitude());
        assertEquals(10.0, rc.getLocationHeight());
        // 无人机飞行中，位置保持原值（由飞行模拟器控制，不被 PUT /location 干预）
        assertEquals(30.671, state.getDroneLatitude());
        assertEquals(104.071, state.getDroneLongitude());
        assertEquals(550.0, state.getDroneElevation());
        assertEquals(50.0, state.getDroneHeight());
        // droneInDock 状态保持不变
        assertFalse(state.isDroneInDock());
