...
class FlightCommandSimulatorTest {
    ...
    void dock3SpecificFieldsParsed() throws Exception {
        ...
        assertTrue(state.isFlightSafetyAdvanceCheck(), "flight_safety_advance_check 应为 true（开启）");
    ...
    void dock1MissingDock3FieldsDefaultsToZero() throws Exception {
        ...
        assertFalse(state.isFlightSafetyAdvanceCheck(), "Dock1 不下发 flight_safety_advance_check，默认应为 false");
    }

    // ==================== TC-FLY-036：flight_safety_advance_check 为 bool 类型 ====================

    @DisplayName("TC-FLY-036：flight_safety_advance_check=true（boolean）反序列化成功，不抛异常")
    @Test
    void flightSafetyAdvanceCheckBooleanTrue() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        Mockito.when(mqtt.isConnected()).thenReturn(true);
        ...
        // 平台下发 flight_safety_advance_check: true（boolean，非 1/0 整数）
        String json = """
                {
                  "flight_id": "FLIGHT-BOOL",
                  "target_latitude": 29.655755,
                  "target_longitude": 106.104925,
                  "target_height": 530,
        ...
        Map<String, Object> reply = simulator.handleTakeoffToPoint(data, "bid-bool");

        // 反序列化成功 → result=0（非 result=1）
        assertNotNull(reply);
        assertEquals(0, reply.get("result"), "flight_safety_advance_check=true 不应导致反序列化失败");
        assertTrue(state.isFlightSafetyAdvanceCheck(), "flight_safety_advance_check 应为 true");
    }

    @DisplayName("TC-FLY-036：flight_safety_advance_check 缺失时默认 false")
    @Test
    void flightSafetyAdvanceCheckMissingDefaultsFalse() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        Mockito.when(mqtt.isConnected()).thenReturn(true);
        ...
        String json = """
                {
                  "flight_id": "FLIGHT-NO-CHECK",
                  "target_latitude": 29.6,
                  "target_longitude": 106.1,
                  "target_height": 530,
                  "security_takeoff_height": 20,
                  "rth_mode": 1,
                  "max_speed": 10
        ...
        Map<String, Object> reply = simulator.handleTakeoffToPoint(data, "bid-no-check");

        assertEquals(0, reply.get("result"));
        assertFalse(state.isFlightSafetyAdvanceCheck(), "缺失 flight_safety_advance_check 时默认应为 false");
    }
    ...
    // ==================== TC-FLY-032: 高度字段椭球高语义 ====================

    @Test
    @DisplayName("TC-FLY-032: takeoff_to_point target_height=椭球高，爬升段 ALT、终点 height=椭球高-机场海拔")
    void takeoffToPointHeightIsEllipsoidalHeight() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        MqttClientManager mqtt = Mockito.mock(MqttClientManager.class);
        Mockito.when(mqtt.isConnected()).thenReturn(true);
        ...
        // 机场椭球高 500（runtimeConfig mock），目标点椭球高 526（相对高度 26），安全起飞高度 20（ALT）
        // 目标点距机场约 50 米（插值 max_speed=12 → ~4.7s 水平到达；垂直 26m/3m/s → ~9.2s 到达）
        String json = """
                {
                  "flight_id": "FLIGHT-032",
                  "target_latitude": 12.23045,
                  "target_longitude": 12.32,
                  "target_height": 526,
                  "security_takeoff_height": 20,
                  "rth_altitude": 50,
                ...
                """;
        simulator.handleTakeoffToPoint(objectMapper.readTree(json), "bid-032");

        // 爬升阶段（wayline_progress 4s + 余量）：height 补齐到 security_takeoff_height（ALT），
        // 插值继续向目标推进（TC-FLY-034 连续插值，非固定快照）
        Thread.sleep(5000);
        assertTrue(state.getDroneHeight() >= 20.0, "爬升段相对高度应已达 security_takeoff_height=20（ALT）");
        assertTrue(state.getDroneHeight() < 26.0, "爬升段不应超过目标相对高度 26");
        assertTrue(state.getDroneElevation() >= 520.0, "爬升段椭球高应已达 机场500+20=520");

        // 到达目标点（垂直 26m/3m/s ≈ 9.2s + 余量）：elevation=target_height（椭球高），height=椭球高-机场海拔
        Thread.sleep(5000);
        assertEquals(526.0, state.getDroneElevation(), 0.001, "终点椭球高应等于 target_height=526");
        assertEquals(26.0, state.getDroneHeight(), 0.001, "终点相对高度应为 526-500=26");
    }

    @Test
    @DisplayName("TC-FLY-032: fly_to_point points[0].height=椭球高，终点 height=椭球高-机场海拔")
    void flyToPointHeightIsEllipsoidalHeight() throws Exception {
        ObjectMapper objectMapper = new ObjectMapper();
        DeviceState state = new DeviceState();
        state.setDroneActivated(true);
        state.setDroneInDock(false);
        ...
        // 目标点椭球高 512（机场椭球高 500 → 相对高度 12），水平距离约 15 米
        // 插值：水平 15m/10m/s ≈ 1.5s 到达；垂直 12m/3m/s = 4s 到达 → 4.5s 时全部到达
        String json = """
                {
                  "fly_to_id": "FLY-032",
                  "points": [{"latitude": 22.0001, "longitude": 113.0001, "height": 512}],
                  "max_speed": 10
                }
                """;
        simulator.handleFlyToPoint(objectMapper.readTree(json), "bid-032");

        // 插值到达目标点后（水平 1.5s + 垂直 4s + 余量）：elevation=512（椭球高），height=512-500=12
        Thread.sleep(4500);
        assertEquals(512.0, state.getDroneElevation(), 0.001, "终点椭球高应等于 points[0].height=512");
        assertEquals(12.0, state.getDroneHeight(), 0.001, "终点相对高度应为 512-500=12");
    }
