...
class FlightCommandSimulatorTest {
    ...
    void takeoffToPointHeightIsEllipsoidalHeight() throws Exception {
        ...
        // 机场椭球高 500（runtimeConfig mock），目标点椭球高 530（相对高度 30），安全起飞高度 20（ALT）
        String json = """
                {
        ...
        // 爬升阶段（wayline_progress，2s 调度 + 2s 间隔）：height=security_takeoff_height（ALT），elevation=机场椭球高+ALT
        Thread.sleep(5000);
        assertEquals(20.0, state.getDroneHeight(), 0.001, "爬升段相对高度应为 security_takeoff_height=20（ALT）");
        assertEquals(520.0, state.getDroneElevation(), 0.001, "爬升段椭球高应为 机场500+20=520");

        // 到达目标点（wayline_ok 6s + task_finish 8s）：elevation=target_height（椭球高），height=椭球高-机场海拔
        Thread.sleep(4000);
        assertEquals(530.0, state.getDroneElevation(), 0.001, "终点椭球高应等于 target_height=530");
        assertEquals(30.0, state.getDroneHeight(), 0.001, "终点相对高度应为 530-500=30");
    }
    ...
    void flyToPointHeightIsEllipsoidalHeight() throws Exception {
        ...
        // 目标点椭球高 530（机场椭球高 500 → 相对高度 30）
        String json = """
                {
                  "fly_to_id": "FLY-032",
                  "points": [{"latitude": 22.1, "longitude": 113.1, "height": 530}],
                  "max_speed": 10
                }
                """;
        simulator.handleFlyToPoint(objectMapper.readTree(json), "bid-032");

        // wayline_ok（2s 调度 + 2s 间隔 = 4s）后位置更新到目标点
        Thread.sleep(4500);
        assertEquals(530.0, state.getDroneElevation(), 0.001, "终点椭球高应等于 points[0].height=530");
        assertEquals(30.0, state.getDroneHeight(), 0.001, "终点相对高度应为 530-500=30");
    }
