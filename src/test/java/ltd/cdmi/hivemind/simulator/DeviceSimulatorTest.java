...
class DeviceSimulatorTest {
    ...
    void droneOsdNotPublishedWhenNotActivated() {
        ...
        // 仅推送 1 次（Dock OSD），Drone OSD 不推送
        Mockito.verify(mqtt, Mockito.times(1)).publish(Mockito.anyString(), Mockito.anyString());
    }

    /**
     * droneActivated=true 时推送 Dock OSD + Drone OSD（共 2 次）。
     */
    @Test
    void droneOsdPublishedWhenActivated() {
        ...
        // 推送 2 次（Dock OSD + Drone OSD）
        Mockito.verify(mqtt, Mockito.times(2)).publish(Mockito.anyString(), Mockito.anyString());
    }
}
