...
class LiveStreamSimulatorTest {
    ...
    private LiveStreamSimulator createSimulator(DockModel dockType) {
        ...
        // 用 mock pusher stub RTMP 可用+startPush 成功，避免依赖测试环境 ffmpeg 是否安装
        FfmpegWhipPusher ffmpegPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(ffmpegPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(ffmpegPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
    ...
    /**
     * TC-LIVE-017：ffmpeg 既不支持 WHIP 也不支持 RTMP 时，返回 513013（协议模拟，不推流）。
     * <p>TC-LIVE-022：仅协议模拟时返回 513013 而非 0，避免平台误以为推流成功反复重试。</p>
     */
    @DisplayName("TC-LIVE-017：WHIP 与 RTMP 均不支持时返回 513013")
    @Test
    void whipAndRtmpBothUnsupported_fallsBackToProtocolSimulation() throws Exception {
        ...
        assertEquals(513013, result.get("result"), "仅协议模拟应返回 513013（TC-LIVE-022）");
        // startPush 不应被调用
    ...
    /**
     * TC-LIVE-018：WebRTC URL 格式异常时（无 stream 参数），返回 513013（协议模拟，不推流）。
     * <p>TC-LIVE-022：仅协议模拟时返回 513013 而非 0。</p>
     */
    @DisplayName("TC-LIVE-018：WebRTC URL 异常时返回 513013")
    @Test
    ...
    // ==================== TC-LIVE-021：设备下线时停止所有直播推流 ====================

    /**
     * TC-LIVE-021：设备下线 → activeStreams 清空 + 所有 ffmpeg 推流进程停止。
     * <p>回归背景：Bug 修复——机场下线（/api/offline）后 ffmpeg 推流进程未停止，
     * 持续以 8Mbps 推流（实测泄漏 8 分钟、470MB+）。</p>
     */
    @DisplayName("TC-LIVE-021：设备下线时停止所有直播推流")
    @Test
    void offline_stopsAllLiveStreams() throws Exception {
        LiveStreamSimulator simulator = createSimulator(DockModel.DOCK3);

        // 建立 2 路活跃推流
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live1\",\"url_type\":1,"
                + "\"video_id\":\"SN/165-0-7/normal-0\",\"video_quality\":3}"));
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live2\",\"url_type\":1,"
                + "\"video_id\":\"SN/165-0-7/zoom-0\",\"video_quality\":3}"));
        assertEquals(2, simulator.getActiveStreams().size());

        // 执行下线清理
        simulator.stopAllStreams();

        // 推流记录清空
        assertTrue(simulator.getActiveStreams().isEmpty(), "下线后 activeStreams 应清空");
        // 再次调用幂等（重复下线不报错）
        simulator.stopAllStreams();
    }

    /**
     * TC-LIVE-021：设备下线 → FfmpegWhipPusher.stopAll() 被调用（停止全部 ffmpeg 进程，含 WHIP 与 RTMP）。
     */
    @DisplayName("TC-LIVE-021：设备下线停止全部 ffmpeg 推流进程")
    @Test
    void offline_invokesFfmpegStopAll() throws Exception {
        RuntimeConfig runtimeConfig = new RuntimeConfig(testMqttProps(), testProps(), new LiveConfigStore());
        runtimeConfig.setDockType(DockModel.DOCK3);

        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        LiveStreamSimulator simulator = new LiveStreamSimulator(
                Mockito.mock(ServiceCommandHandler.class), Mockito.mock(DrcCommandHandler.class),
                runtimeConfig, mockPusher, new DiagnosticLogRecorder());
        ...
        // 验证 ffmpeg 进程清理入口被调用（无论协议模拟还是真实推流模式）
        Mockito.verify(mockPusher).stopAll();
    }

    // ==================== TC-LIVE-022：live_start_push 不支持真实推流时返回 513013 ====================

    /**
     * TC-LIVE-022：url_type 非 RTMP/WebRTC（如 GB28181=3）且 URL 不以 rtmp:// 开头时，
     * 返回 513013 而非 0，避免平台误以为推流成功反复重试。
     */
    @DisplayName("TC-LIVE-022：url_type 不支持真实推流时返回 513013")
    @Test
    void urlTypeUnsupported_returns513013() throws Exception {
        RuntimeConfig runtimeConfig = new RuntimeConfig(testMqttProps(), testProps(), new LiveConfigStore());
        runtimeConfig.setDockType(DockModel.DOCK3);
        ...
        LiveStreamSimulator simulator = new LiveStreamSimulator(
                Mockito.mock(ServiceCommandHandler.class), Mockito.mock(DrcCommandHandler.class),
                runtimeConfig, mockPusher, diagnosticRecorder);

        // url_type=3 (GB28181) + URL 非 rtmp:// → 仅协议模拟
        Map<String, Object> result = simulator.handle("live_start_push", json(
                "{\"url\":\"http://gb28181.example.com/live\",\"url_type\":3,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":3}"));

        assertEquals(513013, result.get("result"), "GB28181 不支持真实推流应返回 513013");
        assertTrue(simulator.getActiveStreams().isEmpty(), "推流未启动不应加入 activeStreams");
        Mockito.verify(mockPusher, Mockito.never()).startPush(
                Mockito.anyString(), Mockito.anyString(), Mockito.anyString(), Mockito.anyInt());
    }

    /**
     * TC-LIVE-022：ffmpeg startPush 返回 false（推流进程启动失败）时返回 513013。
     */
    @DisplayName("TC-LIVE-022：startPush 失败时返回 513013")
    @Test
    void startPushFailed_returns513013() throws Exception {
        RuntimeConfig runtimeConfig = new RuntimeConfig(testMqttProps(), testProps(), new LiveConfigStore());
        runtimeConfig.setDockType(DockModel.DOCK3);
        ...
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
                Mockito.eq(FfmpegWhipPusher.URL_TYPE_RTMP))).thenReturn(false);
        ...
        assertEquals(513013, result.get("result"), "startPush 失败应返回 513013");
        assertTrue(simulator.getActiveStreams().isEmpty(), "推流未启动不应加入 activeStreams");
    }

    // ==================== TC-LIVE-023~025：推流记录 ====================

    /** 构造支持 RTMP 推流的 mock pusher（startPush 成功），用于推流记录测试 */
    private LiveStreamSimulator createSimulatorWithPusher(FfmpegWhipPusher mockPusher) {
        RuntimeConfig runtimeConfig = new RuntimeConfig(testMqttProps(), testProps(), new LiveConfigStore());
        runtimeConfig.setDockType(DockModel.DOCK3);
        return new LiveStreamSimulator(
                Mockito.mock(ServiceCommandHandler.class), Mockito.mock(DrcCommandHandler.class),
                runtimeConfig, mockPusher, new DiagnosticLogRecorder());
    }

    /**
     * TC-LIVE-023：live_start_push 成功 → 推流记录 status=pushing，含下发地址与实际推流地址。
     */
    @DisplayName("TC-LIVE-023：推流成功记录下发地址与实际推流地址")
    @Test
    void pushRecord_success_recordsUrlAndActualUrl() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
                Mockito.eq(FfmpegWhipPusher.URL_TYPE_RTMP))).thenReturn(true);
        LiveStreamSimulator simulator = createSimulatorWithPusher(mockPusher);

        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live\",\"url_type\":1,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":3}"));

        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(1, records.size(), "成功推流应生成一条记录");
        Map<String, Object> record = records.get(0);
        assertEquals(VIDEO_ID, record.get("video_id"));
        assertEquals("rtmp://192.168.1.1:8080/live", record.get("url"), "应记录平台下发地址");
        assertEquals(1, record.get("url_type"));
        assertEquals("rtmp://192.168.1.1:8080/live", record.get("actual_url"), "正常推流实际地址与下发地址相同");
        assertEquals("pushing", record.get("status"));
        assertEquals(3, record.get("quality"));
        assertFalse(record.containsKey("fail_reason"), "成功记录不应有失败原因");
        assertNotNull(record.get("start_time"), "应记录开始时间");
        assertNull(record.get("end_time"), "推流中不应有结束时间");
    }

    /**
     * TC-LIVE-023：推流失败（url_type 不支持）→ 失败记录保留 + 失败原因，services_reply 仍返回 513013。
     */
    @DisplayName("TC-LIVE-023：推流失败记录保留并含失败原因")
    @Test
    void pushRecord_failed_keepsRecordWithReason() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isRealPushEnabled()).thenReturn(true);
        LiveStreamSimulator simulator = createSimulatorWithPusher(mockPusher);
        ...
        assertEquals(513013, result.get("result"), "TC-LIVE-022 行为不变");
        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(1, records.size(), "失败记录应保留（不从记录列表移除）");
        Map<String, Object> record = records.get(0);
        assertEquals("failed", record.get("status"));
        assertEquals("http://gb28181.example.com/live", record.get("url"), "失败记录仍含平台下发地址（排错核心信息）");
        String failReason = String.valueOf(record.get("fail_reason"));
        assertTrue(failReason.contains("url_type=3"), "失败原因应指明 url_type，实际: " + failReason);
    }

    /**
     * TC-LIVE-023：同一 video_id 重复 start_push（幂等）→ 旧记录被最新一次替换。
     */
    @DisplayName("TC-LIVE-023：重复 start_push 同 video_id 记录被替换")
    @Test
    void pushRecord_duplicateStartPush_replacesOldRecord() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
                Mockito.eq(FfmpegWhipPusher.URL_TYPE_RTMP))).thenReturn(true);
        ...
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live/old\",\"url_type\":1,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":3}"));
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live/new\",\"url_type\":1,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":0}"));

        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(1, records.size(), "同 video_id 仅一条记录（幂等替换）");
        assertEquals("rtmp://192.168.1.1:8080/live/new", records.get(0).get("url"), "记录应为最新一次下发");
        assertEquals(0, records.get(0).get("quality"));
    }

    /**
     * TC-LIVE-024：WHIP→RTMP 降级推流 → 记录下发地址（WebRTC）与实际推流地址（RTMP）。
     */
    @DisplayName("TC-LIVE-024：WHIP 降级 RTMP 记录实际推流地址")
    @Test
    void pushRecord_whipDegradedToRtmp_recordsBothUrls() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_WEBRTC)).thenReturn(false);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
        ...
        Map<String, Object> record = records.get(0);
        assertEquals(whipUrl, record.get("url"), "应记录平台下发的 WebRTC 地址");
        assertEquals(4, record.get("url_type"));
        assertEquals("rtmp://172.30.48.1:1935/live/test_stream", record.get("actual_url"),
                "应记录降级后的实际 RTMP 推流地址");
        assertEquals(1, record.get("actual_url_type"), "实际推流类型应为 RTMP");
        assertNotEquals(record.get("url"), record.get("actual_url"), "降级场景两个地址不同（前端据此显示降级标注）");
    }

    /**
     * TC-LIVE-025：live_stop_push → 记录状态更新为 stopped 并记录结束时间。
     */
    @DisplayName("TC-LIVE-025：live_stop_push 更新记录状态为 stopped")
    @Test
    void pushRecord_stopPush_marksStopped() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
                Mockito.eq(FfmpegWhipPusher.URL_TYPE_RTMP))).thenReturn(true);
        ...
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live\",\"url_type\":1,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":3}"));
        simulator.handle("live_stop_push", json("{\"video_id\":\"" + VIDEO_ID + "\"}"));

        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(1, records.size(), "记录保留（仅状态更新，不删除）");
        assertEquals("stopped", records.get(0).get("status"));
        assertNotNull(records.get(0).get("end_time"), "停止后应记录结束时间");
        assertTrue(simulator.getActiveStreams().isEmpty(), "activeStreams 行为不变（TC-LIVE-003）");
    }

    /**
     * TC-LIVE-025：设备下线（stopAllStreams）→ 所有 pushing 记录更新为 stopped。
     */
    @DisplayName("TC-LIVE-025：设备下线所有推流记录标记 stopped")
    @Test
    void pushRecord_stopAllStreams_marksAllStopped() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isPushAvailable(FfmpegWhipPusher.URL_TYPE_RTMP)).thenReturn(true);
        Mockito.when(mockPusher.startPush(Mockito.anyString(), Mockito.anyString(), Mockito.anyString(),
                Mockito.eq(FfmpegWhipPusher.URL_TYPE_RTMP))).thenReturn(true);
        ...
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live/a\",\"url_type\":1,"
                + "\"video_id\":\"" + VIDEO_ID + "\",\"video_quality\":3}"));
        simulator.handle("live_start_push", json(
                "{\"url\":\"rtmp://192.168.1.1:8080/live/b\",\"url_type\":1,"
                + "\"video_id\":\"SN2/39-0-7/zoom-0\",\"video_quality\":3}"));
        simulator.stopAllStreams();

        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(2, records.size(), "记录保留（供下线后排查）");
        for (Map<String, Object> record : records) {
            assertEquals("stopped", record.get("status"), "所有 pushing 记录应更新为 stopped");
            assertNotNull(record.get("end_time"));
        }
    }

    /**
     * TC-LIVE-025：推流记录有界（保留最近 20 条），超出时最旧记录淘汰，新记录在最前。
     */
    @DisplayName("TC-LIVE-025：推流记录上限 20 条且最新在前")
    @Test
    void pushRecord_boundedTo20_latestFirst() throws Exception {
        FfmpegWhipPusher mockPusher = Mockito.mock(FfmpegWhipPusher.class);
        Mockito.when(mockPusher.isRealPushEnabled()).thenReturn(true);
        // isPushAvailable 未 stub（默认 false）→ 全部生成失败记录，无需真实推流
        LiveStreamSimulator simulator = createSimulatorWithPusher(mockPusher);

        for (int i = 1; i <= 25; i++) {
            simulator.handle("live_start_push", json(
                    "{\"url\":\"rtmp://192.168.1.1:8080/live/" + i + "\",\"url_type\":1,"
                    + "\"video_id\":\"SN" + i + "/39-0-7/normal-0\",\"video_quality\":0}"));
        }

        List<Map<String, Object>> records = simulator.getPushRecords();
        assertEquals(20, records.size(), "记录应限制在 20 条");
        assertEquals("SN25/39-0-7/normal-0", records.get(0).get("video_id"), "最新记录应在最前");
        assertEquals("SN6/39-0-7/normal-0", records.get(19).get("video_id"), "最旧记录（SN1~SN5）应被淘汰");
    }
}
