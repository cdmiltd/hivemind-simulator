...
class LiveStreamSimulatorTest {
    ...
    private SimulatorProperties testProps(DeviceType dockType) {
        return new SimulatorProperties(
                new SimulatorProperties.Device(
                        "SIM-DOCK-001", "SIM-DRONE-001",
                        dockType, DeviceType.M4TD,
                        "org", "code", "license"),
    ...
    private LiveStreamSimulator createSimulator(DeviceType dockType) {
        RuntimeConfig runtimeConfig = new RuntimeConfig(testMqttProps(), testProps(dockType));
        ServiceCommandHandler commandHandler = Mockito.mock(ServiceCommandHandler.class);
        return new LiveStreamSimulator(testProps(dockType), commandHandler, runtimeConfig);
    }
