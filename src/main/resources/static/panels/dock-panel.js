...
function registerDockPanel(app) {
    ...
    app.component('dock-panel', {
        setup() {
            ...
            // 风向 8 方位枚举（DJI 协议 wind_direction：1=正北…8=西北，仅飞行器 OSD 上报）
            ...
            const psdk = reactive({
                psdkIndex: 2,             // psdk_index 必填，默认 2（与后端 PsdkConstants.SPEAKER_INDEX 保持一致，喊话器设备索引）
                state: null,              // 查询返回的状态
            ...
            // 飞行器三态（editState.droneInDock + droneActivated 组合）：
            // 在舱 / 飞行中（不在舱+已激活，推送 drone OSD）/ 离舱（不在舱+休眠，关机不推送 OSD）
            // 与下方 dronePosition 实时状态卡片互补：此处是用户编辑态，下方是后端实时反馈
            const isDroneFlying = computed(() =>
                !ctx.editState.droneInDock && ctx.editState.droneActivated);
            const flightModeLabel = computed(() => {
                if (ctx.editState.droneInDock) return '在舱';
                return ctx.editState.droneActivated ? '飞行中' : '离舱';
            });
            const flightModeTagType = computed(() => {
                if (ctx.editState.droneInDock) return 'info';
                return ctx.editState.droneActivated ? 'warning' : 'info';
            });
            ...
            return {
                ...
                // 飞行器三态
                isDroneFlying, flightModeLabel, flightModeTagType,
                // Dock 专属方法
        ...
        template: `
            ...
            <!-- 状态模拟（Dock 版：机场环境组 + 飞行器组） -->
            <el-card class="right-card" header="机场状态模拟">
                <el-form label-width="72px" size="small">
                    <!-- 机场环境组（字段对应机场 OSD：temperature/humidity/wind_speed/cover_state/silent_mode） -->
                    <div class="state-group-title">机场环境</div>
                    ...
                    <!-- 飞行器组（字段对应飞行器 OSD：capacity_percent/wind_speed/wind_direction；电量也用于机场 OSD drone_charge_state） -->
                    <div class="state-group-title">飞行器</div>
                    <el-form-item label="在舱/激活">
                        <el-switch v-model="editState.droneInDock" @change="saveState"></el-switch>
                        <el-tag size="small" :type="flightModeTagType" :class="{ 'drone-flying': isDroneFlying }" effect="dark" style="margin-left: 4px;">
                            <span class="drone-flying-dot" v-if="isDroneFlying"></span>{{ flightModeLabel }}
                        </el-tag>
                        <el-radio-group v-model="editState.droneActivated" size="small" @change="saveState" style="margin-left: 12px;">
                    ...
                    <el-form-item label="风速(m/s)">
                        <el-input-number v-model="editState.droneWindSpeed" :min="0" :step="0.5" size="small" @change="saveState"></el-input-number>
                    </el-form-item>
                    <el-form-item label="风向">
                        <el-select v-model="editState.windDirection" :disabled="!editState.droneActivated" @change="saveState" size="small" style="width: 140px;">
                            <el-option v-for="opt in windDirectionOptions" :key="opt.value" :label="opt.label" :value="opt.value"></el-option>
                        </el-select>
                        <span class="state-field-hint" v-if="!editState.droneActivated">需先激活飞行器</span>
                    </el-form-item>
                </el-form>
            </el-card>
            ...
            <el-dialog v-model="mediaTriggerVisible" title="触发媒体上报" width="420px" :close-on-click-modal="false">
                <el-form :model="mediaTriggerForm" label-width="90px" size="small">
                    ...
                    <div class="el-form-item" role="group" aria-label="说明">
                        <label class="el-form-item__label" style="width: 90px;">说明</label>
            ...
            <el-card class="right-card">
                ...
                <div style="margin-top: 12px; padding-top: 10px; border-top: 1px dashed #e4e7ed;">
                    ...
                    <el-descriptions :column="1" border size="small">
                        ...
                        <el-descriptions-item label="海拔(m)">{{ dronePosition.activated ? dronePosition.elevation.toFixed(1) : '-' }}</el-descriptions-item>
