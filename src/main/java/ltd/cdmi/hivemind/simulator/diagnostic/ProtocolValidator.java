...
public final class ProtocolValidator {
    ...
    /**
     * 主动校验 property/set 下行消息的必填字段。
     * <p>property/set 报文格式与 services 不同：{@code {tid, bid, timestamp, data}}，无 method 字段
     * （method 是 services 指令的必填字段，属性设置按物模型 topic 寻址，不需要 method）。
     * @param node 已解析的 JSON 消息（非 null）
     * @return null=校验通过；DiagnosticCode=第一个错误
     */
    public static DiagnosticCode validatePropertySetFields(JsonNode node) {
        // P-6：必填字段校验（property/set 仅要求 tid/bid，无 method）
        List<String> missing = new ArrayList<>();
        if (!node.has("tid")) missing.add("tid");
        if (!node.has("bid")) missing.add("bid");
        if (!missing.isEmpty()) {
