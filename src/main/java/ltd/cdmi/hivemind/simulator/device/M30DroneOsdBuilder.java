 ...
 * <p>M30 家族特有字段：rc_lost_action（遥控器失控动作）、cameras 数组、rid_state（RID 工作状态）、
...
public class M30DroneOsdBuilder extends AbstractDroneOsdBuilder {
    ...
    protected void appendDroneSpecific(OsdContext ctx, Map<String, Object> data) {
        ...
        data.put(s.convertKey("rc_lost_action"), 2);        // 返航（pushMode=0, rw）
        data.put(s.convertKey("cameras"), buildCameras(ctx));
        // rid_state — RID 工作状态（仅 M30/M30T 有，pushMode=0, r）
        data.put(s.convertKey("rid_state"), true);          // true=正常
