 ...
 * <p>例外：{@link Sn#uniqueEnabled()} 唯一 SN 模式（多实例部署时生成并持久化实例唯一 SN，
 * 见设计文档 §7.1 SN 生成机制）。</p>
 */
...
public record SimulatorProperties(
    ...
    /**
     * SN 生成模式配置（设计文档 §7.1）。
     * <p>unique-enabled=false（缺省）：SN 取设备型号 defaultSn()，单实例调试用，行为与历史版本一致；
     * unique-enabled=true：生成并持久化实例唯一 SN（defaultSn 前缀 + 随机后缀），
     * 供 docker-compose-multi 等多实例部署避免 SN 冲突（TDD-SPEC TC-REG-028~031）。</p>
     */
    public record Sn(
            boolean uniqueEnabled
    ) {}
