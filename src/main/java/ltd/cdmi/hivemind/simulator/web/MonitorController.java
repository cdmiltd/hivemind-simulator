 ...
 */
@Tag(name = "监控器", description = "MQTT 消息订阅、设备状态查询与指令下发，用于排查平台侧协议交互问题")
@RestController
...
public class MonitorController {
    ...
    private final LiveStreamSimulator liveStreamSimulator;
    ...
    public MonitorController(MonitorService monitorService,
                             ...
                             LiveStreamSimulator liveStreamSimulator) {
        ...
        this.liveStreamSimulator = liveStreamSimulator;
    }
    ...
    public Map<String, Object> liveStreams() {
        // 合并 MQTT 监听数据 + 模拟器实时直播状态
        Map<String, List<Map<String, Object>>> streams = monitorService.getLiveStreams();
        // 回退：当 MQTT 监听无数据时，从模拟器获取当前活跃推流
        if (streams.isEmpty()) {
            List<Map<String, Object>> activeStreams = liveStreamSimulator.getActiveStreams();
            if (!activeStreams.isEmpty()) {
                String sn = runtimeConfig.getGatewaySn();
                streams = new LinkedHashMap<>();
                streams.put(sn, activeStreams);
            }
        }
        Map<String, Object> result = new LinkedHashMap<>();
    ...
    public Map<String, Object> command(@RequestBody Map<String, Object> body) {
        ...
        try {
            Map<String, Object> reply = monitorService.sendCommand(sn, method, data);
            result.put("success", true);
            result.put("reply", reply);
            // 从 reply 中提取执行状态（DJI 协议：services_reply 的 data 中含 status 字段）
            Object replyData = reply.get("data");
            if (replyData instanceof Map) {
                Object status = ((Map<?, ?>) replyData).get("status");
                if (status != null) {
                    result.put("status", status);
                }
            }
            // 超时场景
            if ("timeout".equals(reply.get("status"))) {
                result.put("success", false);
                result.put("message", reply.get("message"));
            }
        } catch (Exception e) {
            result.put("success", false);
