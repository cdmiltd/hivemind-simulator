...
import ltd.cdmi.simulator.handler.FfmpegWhipPusher;
import ltd.cdmi.simulator.handler.FfmpegInstaller;
import ltd.cdmi.simulator.handler.FlightCommandSimulator;
import ltd.cdmi.simulator.handler.HmsSimulator;
import ltd.cdmi.simulator.handler.LiveStreamSimulator;
...
public class SimulatorController {
    ...
    public Map<String, Object> deviceInfo() {
        Map<String, Object> result = new LinkedHashMap<>();
        result.put("dockSn", props.device().dockSn());
        result.put("droneSn", props.device().droneSn());
     ...
     * 类型名称与 {@link ltd.cdmi.simulator.handler.HmsSimulator.AlarmType} 枚举常量一致（大小写不敏感）。</p>
     * <p>遵循"业务逻辑返回明确拒绝原因而非抛异常"约定：
